...
def cal_2th_moment(unit_right, rho_batch, x_batch, updeg, pauli_batch):
    ...
    # Expand to broadcast
    unit_exp = unit_right[:, None, :, :]             # (N, 1, d, d)
    ...
    return tr_square[:, :, None] * tr2_powers / measure_factor[None, None, :]
...
if __name__ == "__main__":
    ...
    # Initial states
    rho_arr = torch.zeros((plot_num, dim, dim), dtype=dtype, device=device)
    ...
    var_gp_arr = torch.zeros((num_batch, X_L_arr.shape[0], updeg), device=device)
    m2_gp_arr = torch.zeros_like(var_gp_arr)

    angle_list = gen_angle_range(dim)

    theoretical_var = torch.zeros((X_L_arr.shape[0], updeg))
    for i in range(updeg):
        theoretical_var[:, i] = torch.real(theoretical_cal.theoretical_calculation(X_L_arr, measure_word_arr, i))
    ...
    for s in range(num_batch):
        ...
        result = cal_2th_moment(env_right, rho_arr, X_L_arr, updeg, measure_word_arr) * factor_s[:,None, None]
    ...
    torch.save(theoretical_var, 'theoretical_var_3bits_v2.pt')
    torch.save(numerical_var, 'numerical_var_3bits_v2.pt')
    torch.save(yerr_arr, 'yerr_3bits_v2.pt')